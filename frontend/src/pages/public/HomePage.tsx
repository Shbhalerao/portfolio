  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gray-50 text-text-dark">
      ...
      `}>
        ...
        {/* Achievement Cards (Responsive: stacked for small screens, row for md+) */}
        <div className="w-full max-w-xs mx-auto">
          {/* Mobile: one per line, uniform size */}
          <div className="block md:hidden w-full space-y-2">
            {[0, 2, 1].map((i) => (
              <div
                key={i}
                className="flex items-center gap-2 bg-white/80 border border-gray-200 rounded-full px-3 py-2 shadow-sm hover:shadow-md transition-all duration-200 w-full min-w-0"
                style={{ minWidth: 0 }}
              >
                <span className={`w-7 h-7 flex items-center justify-center rounded-full ${achievementCards[i].circleBg} shadow`}>
                  <IconContext.Provider value={{ className: `${achievementCards[i].iconColor} text-base` }}>
                    {achievementCards[i].icon}
                  </IconContext.Provider>
                </span>
                <span className="flex flex-col items-start">
                  <span className="text-xs font-bold text-text-dark leading-tight">{achievementCards[i].text}</span>
                  <span className="text-[10px] text-gray-500 leading-tight">{achievementCards[i].subText}</span>
                </span>
              </div>
            ))}
          </div>
          ...
          <div className="hidden md:flex justify-center gap-2 w-full">
            {achievementCards.map((card, index) => (
              <div
                key={index}
                className="flex items-center gap-2 bg-white/80 border border-gray-200 rounded-full px-3 py-2 shadow-sm hover:shadow-md transition-all duration-200 min-w-[90px] max-w-xs w-full md:w-auto"
                style={{ minWidth: 240 }}
              >
